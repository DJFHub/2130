...
void  handleTCPClient(int client_socket)
{
    char  x_cordinate[BUF_SIZE], y_cordinate[BUF_SIZE] 
    , letter[BUF_SIZE], playing[BUF_SIZE];
    int  recvMsgSize; 
    
    //recieve message from client 
     if (recvMsgSize = recv(client_socket, playing, BUF_SIZE, 0)!= recvMsgSize)
        printf("\nrecv() failed\n");
    playing[BUF_SIZE] = '\0'; 
    if (recvMsgSize = recv(client_socket, x_cordinate, BUF_SIZE, 0)!= recvMsgSize)
        printf("\nrecv() failed\n");
    x_cordinate[BUF_SIZE] = '\0';
    if (recvMsgSize = recv(client_socket, y_cordinate, BUF_SIZE, 0)!= recvMsgSize)
        printf("\nrecv() failed\n");
    y_cordinate[BUF_SIZE] = '\0';
    if (recvMsgSize = recv(client_socket,letter, BUF_SIZE, 0)!= recvMsgSize)
        printf("\nrecv() failed\n");
    letter[BUF_SIZE] = '\0';

        
    //we need to make the play 
    if (strcmp("n",playing)!=0)
     makePlay(atoi(x_cordinate),atoi(y_cordinate), &letter);
    else
        running = 0 ; 
